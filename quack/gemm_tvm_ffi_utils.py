...
def make_fake_varlen_args(varlen_m, varlen_k, gather_A, aidx_len, lens_k=False):
    ...
    # mLensK is shape (l,) — one entry per batch — whereas mCuSeqlensK is
    # (l+1,). Use an independent sym_int so the compile shim doesn't impose
    # a same-shape constraint on the two tensors.
